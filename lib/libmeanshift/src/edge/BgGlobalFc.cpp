...
void timer_stop()
{
   ...
   seconds = static_cast<unsigned long>( (timeend-timestart)/CLOCKS_PER_SEC );
   milliseconds = static_cast<unsigned long>( ((100*(timeend-timestart))/CLOCKS_PER_SEC) - 100*seconds );
   bgLog("timer stop, elapsed %d.%d seconds.\n", seconds, milliseconds);
}
